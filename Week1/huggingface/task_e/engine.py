...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq, processor, scaler=None):
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # NO et fiïs només de _to_device: força al device real del model
        inputs = _force_hf_inputs_to_model_device(inputs, model)

        # (debug ràpid: descomenta 1 cop si vols)
...
def evaluate(model, data_loader, device, processor):
    ...
    for images, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        # Ensure every tensor inside the HF BatchFeature lives on the same device
        # as the model (similar to the training loop). Without this, the processor
        # may leave pixel_values on CPU while the model weights are on CUDA,
        # producing the FloatTensor / cuda.FloatTensor type mismatch observed
        # during evaluation.
        inputs = _force_hf_inputs_to_model_device(inputs, model)
    ...
    # average model_time per batch/image (global_avg uses total/count)
    avg_model_time = metric_logger.model_time.global_avg if "model_time" in metric_logger.meters else None

    return coco_evaluator, avg_model_time
