...
def train_one_epoch(
    ...
    for batch_idx, (img, caption, *_) in enumerate(dataloader):
        ...
        with torch.amp.autocast('cuda', enabled=(device == 'cuda')):
            pred = model(img, ground_truth=caption, epoch=epoch)
            loss = criterion(pred, caption[:, 1:])
...
def eval_epoch(
    ...
    for batch_idx, (img, caption, img_names, all_captions) in enumerate(dataloader):
        ...
        with torch.amp.autocast('cuda', enabled=(device == 'cuda')):
            pred      = model(img)
            loss_val  = criterion(pred, caption[:, 1:])

        running_loss += loss_val.item()

        pred_ids = pred.argmax(dim=1).cpu().tolist() # (B, T-1)
        cap_ids  = caption.cpu().numpy()

        all_preds_ids.extend(pred_ids)
        all_refs_texts.extend(all_captions) # list of lists of strings

        # Collect per-image predictions for the eval framework
        for pred_seq, img_name, caps in zip(pred_ids, img_names, all_captions):
            predictions.append({
                'image':      img_name,
                'prediction': decode_caption(pred_seq, vocab),
    ...
    metrics  = metric(all_preds_ids, all_refs_texts, vocab)
...
def train(
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_metrics, val_preds = eval_epoch(model, criterion, metric, dataloader_val, device, vocab, epoch)
