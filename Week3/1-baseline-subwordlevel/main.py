"""Entry point for the subword-level image-captioning model (ResNet-18 + GRU)."""
import argparse
import json
import os
...
def make_dataloader(samples, img_path, shuffle):
    ds = VizWizDataset(samples, img_path)
    return DataLoader(
        ds,
...
def parse_args():
    parser = argparse.ArgumentParser(description='ResNet-18 + GRU subword-level image captioning')
    parser.add_argument(
...
def main():
    ...
    # Initialise subword tokenizer (trains SP model from train captions if not yet done)
    tok.init(train_json=TRAIN_JSON)
    ...
    dl_train = make_dataloader(train_samples, TRAIN_IMG_PATH, shuffle=True)
    dl_val   = make_dataloader(val_samples,   VAL_IMG_PATH,   shuffle=False)

    # Model
    num_layers = args.num_decoder_layers if args.num_decoder_layers is not None else NUM_DECODER_LAYERS
    model     = BaselineModel(freeze_encoder=FREEZE_ENCODER, num_decoder_layers=num_layers).to(DEVICE)
    ...
    summary = {
        'model':       'ResNet18_GRU_subwordlevel',
