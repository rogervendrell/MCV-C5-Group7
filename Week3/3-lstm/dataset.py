...
class VizWizDataset(Dataset):
    ...
    def __init__(self, samples: list[dict], img_path: str, vocab, max_len: int = TEXT_MAX_LEN):
    ...
    def encode_caption(self, caption: str) -> torch.Tensor:
        words = ['<SOS>'] + caption.lower().split() + ['<EOS>']
        words = words[:self.max_len]

        indices = [self.vocab.encode(w) for w in words]
        
        while len(indices) < self.max_len:
            indices.append(self.vocab.encode('<PAD>'))
    ...
    def __getitem__(self, idx: int):
        ...
        if (cap_tensor >= len(self.vocab)).any():
            print(f"Found invalid index! Max index: {cap_tensor.max()}, Vocab size: {len(self.vocab)}")
