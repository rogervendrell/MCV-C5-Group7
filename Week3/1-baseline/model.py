...
from vocabulary import NUM_CHAR, char2idx, TEXT_MAX_LEN
...
class BaselineModel(nn.Module):
    ...
    def forward(self, img: torch.Tensor, ground_truth: torch.Tensor | None = None) -> torch.Tensor:
        ...
        Returns:
            logits: (B, NUM_CHAR, TEXT_MAX_LEN)
        """
        ...
        sos = torch.tensor(char2idx['<SOS>'], device=img.device)
        ...
        res = self.proj(res)           # (B, T, NUM_CHAR)
        res = res.permute(0, 2, 1)     # (B, NUM_CHAR, T)
        return res
