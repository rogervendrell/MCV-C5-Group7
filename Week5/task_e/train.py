...
def train_llama_one_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if batch_idx % PRINT_EVERY == 0:
            ...
            remaining = len(dataloader) - done
            eta       = elapsed / done * remaining if done > 0 else 0.0
            print(
                f"  epoch {epoch} | batch {batch_idx}/{len(dataloader)} "
                f"| loss {loss.item():.4f} | {elapsed:.1f}s elapsed | ETA {eta:.1f}s",
                flush=True,
            )
...
def eval_llama_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if batch_idx % PRINT_EVERY == 0:
            ...
            eta      = elapsed / done * remaining if done > 0 else 0.0
            print(
                f"  {ep}val batch {batch_idx}/{len(dataloader)} "
...
def save_resume_checkpoint(
    ...
    """Save full training state so a job can be resumed exactly."""
    torch.save({
        'epoch':         epoch,
...
def load_resume_checkpoint(path: str, model, optimizer, scaler, device: str) -> dict:
    """Load a resume checkpoint; returns the saved training state dict."""
...
def train_llama(
    ...
    During training, validation uses greedy decoding (num_beams=1) to keep
    each epoch fast.  Beam search (from generation_kwargs) is only applied
    on the final best-model re-evaluation after training ends.

    Pass resume_checkpoint to continue a previously interrupted job from the
    exact epoch where it stopped.
    """
    scaler = torch.amp.GradScaler('cuda', enabled=(device == 'cuda'))

    # Greedy kwargs for mid-training validation — avoids expensive beam search.
    greedy_kwargs = {
        **{k: v for k, v in (generation_kwargs or {}).items()
           if k not in ('num_beams',)},
        'num_beams': 1,
    }
    ...
    resume_path = resume_checkpoint or os.path.join(os.path.dirname(save_path), 'resume.pt')
    if os.path.exists(resume_path):
        print(f"Resuming from checkpoint: {resume_path}", flush=True)
        state      = load_resume_checkpoint(resume_path, model, optimizer, scaler, device)
        start_epoch   = state['epoch'] + 1
        best_val_loss = state['best_val_loss']
        best_epoch    = state['best_epoch']
        no_improve    = state['no_improve']
        history       = state['history']
        print(f"  Resuming at epoch {start_epoch} | best val loss so far: {best_val_loss:.4f}", flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        val_loss, val_metrics, val_preds = eval_llama_epoch(
            ...
            device=device,
            epoch=epoch,
            generation_kwargs=greedy_kwargs,
        )
        ...
        save_resume_checkpoint(
            ...
            epoch, best_val_loss, best_epoch, no_improve, history,
        )
